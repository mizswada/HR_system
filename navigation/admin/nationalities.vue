...
<template>
  <div class="mb-4">
<h1 class="text-2xl font-bold">Nationalities</h1>
  ...
  <rs-modal
    :title="modalType == 'edit' ? 'Edit Nationalities' : 'Add Nationalities'"
    ok-title="Save"
    :ok-callback="saveNational"
    v-model="showModal"
    :overlay-close="false"
  >
    <FormKit
      type="text"
      v-model="showModalForm.nationalName"
      name="nationalName"
      label="Name"
  ...
  <rs-modal
    ...
    ok-title="Yes"
    cancel-title="No"
    :ok-callback="deleteNational"
    v-model="showModalDelete"
    :overlay-close="false"
  >
    <p>
      Are you sure want to delete this skill ({{
        showModalDeleteForm.nationalName
      }})?
    </p>
  </rs-modal>
</template>