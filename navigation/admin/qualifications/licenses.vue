...
<h1 class="text-2xl font-bold">Licenses</h1>
  ...
  <rs-modal
    :title="modalType == 'edit' ? 'Edit License' : 'Add License'"
    ok-title="Save"
    :ok-callback="saveLicense"
    v-model="showModal"
    :overlay-close="false"
  >
    <FormKit
      type="text"
      v-model="showModalForm.licenseLevel"
      name="licenseLevel"
      label="Name"
  ...
  <rs-modal
    ...
    ok-title="Yes"
    cancel-title="No"
    :ok-callback="deleteLicense"
    v-model="showModalDelete"
    :overlay-close="false"
  >
    <p>
      Are you sure want to delete this skill ({{
        showModalDeleteForm.licenseLevel
      }})?
    </p>
  </rs-modal>
</template>