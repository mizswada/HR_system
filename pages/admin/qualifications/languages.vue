...
<h1 class="text-2xl font-bold">Languages</h1>
  ...
  <rs-modal
    :title="modalType == 'edit' ? 'Edit Language' : 'Add Language'"
    ok-title="Save"
    :ok-callback="saveLanguage"
    v-model="showModal"
    :overlay-close="false"
  >
    <FormKit
      type="text"
      v-model="showModalForm.languageName"
      name="languageName"
      label="Name"
  ...
  <rs-modal
    ...
    ok-title="Yes"
    cancel-title="No"
    :ok-callback="deleteLanguage"
    v-model="showModalDelete"
    :overlay-close="false"
  >
    <p>
      Are you sure want to delete this skill ({{
        showModalDeleteForm.languageName
      }})?
    </p>
  </rs-modal>
</template>