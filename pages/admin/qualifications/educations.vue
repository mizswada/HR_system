...
<h1 class="text-2xl font-bold">Education</h1>
  ...
  <rs-modal
    :title="modalType == 'edit' ? 'Edit Education' : 'Add Education'"
    ok-title="Save"
    :ok-callback="saveEducation"
    v-model="showModal"
    :overlay-close="false"
  >
    <FormKit
      type="text"
      v-model="showModalForm.educationLevel"
      name="educationLevel"
      label="Level"
  ...
  <rs-modal
    ...
    ok-title="Yes"
    cancel-title="No"
    :ok-callback="deleteEducation"
    v-model="showModalDelete"
    :overlay-close="false"
  >
    <p>
      Are you sure want to delete this skill ({{
        showModalDeleteForm.educationLevel
      }})?
    </p>
  </rs-modal>
</template>